...
module.exports = function make_process_episode() {
  return async function ingest_process_episode(this: any, msg: any, meta: any) {
    const seneca = this
    ...
    if (doUpdate) {
      ...
      if (null != episode_id) {
        ...
        if (null === episodeEnt) {
          out.why = 'podcast-not-found'
          debug && debug('PROCESS-FAIL-EPISODEID-NOTFOUND',
            batch, mark, podcast_id, episode_id, out)
          return out
        }
      }
      ...
      if (doIngest) {
        ...
        if (doExtract) {
          ...
          for (; tryI < 7; tryI++) {
            info.ok = false
            slog('EPISODE-QUERY',
              batch, podcastEnt.id, episodeEnt.id, episodeEnt.guid, tryI, episodeEnt.title)
            ...
            try {
              ...
              debug && debug('PROCESS-FAIL-QUERY-JSON',
                batch, mark, podcast_id, episodeEnt.id, e.message, out, tryI, processRes.answer)
            }
          ...
          if (!info.ok) {
            slog('EPISODE-FAIL', batch, podcastEnt.id, episodeEnt.id, episodeEnt.guid,
              episodeEnt.title, 'QUERY-JSON', tryI)
        ...
        if (customRes.ok) {
          ...
          debug && debug('PROCESS-FAIL-CUSTOM',
      ...
      debug && debug('PROCESS-EPISODE-SAVE',
